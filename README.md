...
从16bit天文图像中检测星点坐标，集成PSF拟合过滤、FWHM剪裁、圆度过滤和饱和星点检测，输出通过所有过滤的真实星点坐标列表。
...
### 饱和星点检测
![饱和星点检测示例](test_output/example2.jpg)

*绿色十字 = 正常PSF拟合星点，红色圆圈 = 饱和星点（半动态范围二值化+连通域+圆拟合检测）*
...
- **饱和星点检测**：半动态范围二值化+连通域分析+圆拟合，提取饱和星质心
...
输入: uint16图像
  ...
  │
  ├─ ── 饱和星点检测 (sdet_detect_ex) ──
  ├─ 12. 半动态范围二值化: threshold = min + (max-min)×0.5
  ├─ 13. 连通域分析 → 饱和区域
  ├─ 14. 面积过滤: < 5px → 去噪
  ├─ 15. 圆拟合: 加权质心 + 等效半径 + 圆度(4π×area/perimeter²) > 0.3
  ├─ 16. 3σ截断: 面积 > median + 3×MAD → 去除异常大区域
  ├─ 17. 去重: 与正常检测星距离<2px的饱和星不重复输出
  │
  └─ 输出: 饱和星(前) + 正常PSF星(后) 坐标列表
```
...
// 扩展检测：输出坐标 + flux + 饱和标记
// 饱和星排在最前面 (flux=-1.0, saturated=1)
// 正常PSF星排在后面 (saturated=0)
int sdet_detect_ex(StarDetectorHandle handle,
                   const uint16_t *image, int width, int height,
                   double **out_x, double **out_y,
                   float **out_flux, int **out_saturated,
                   int *out_count);
...
| out_x | double[] | 星点X坐标 |
| out_y | double[] | 星点Y坐标 |
| out_flux | float[] | 星点flux值，饱和星为 -1.0 |
| out_saturated | int[] | 饱和标记，1=饱和星，0=正常PSF星 |
| out_count | int* | 总星点数（饱和+正常） |
...
```python
det = StarDetector()
result = det.detect_debug_image(image, "debug_output.png")
# 生成标注图像: 绿色十字=正常星, 红色圆圈=饱和星
det.close()
```
...
亮星在图像中形成饱和区域，PSF拟合无法获得准确质心。饱和星点检测流程：

1. **半动态范围二值化**：threshold = img_min + (img_max - img_min) × 0.5
2. **连通域分析**：找到所有高亮连通区域
3. **噪声过滤**：面积 < 5px 的连通域为单点噪声，去除
4. **圆拟合**：计算加权质心(cx, cy)、等效半径 r=√(area/π)、圆度=4π×area/perimeter²
5. **圆度过滤**：圆度 > 0.3 的保留（星点近似圆形）
6. **3σ截断**：面积 > median + 3×MAD 的异常大区域去除（星云/星系核心）
7. **去重**：与正常PSF检测星距离 < 2px 的饱和星不重复输出

饱和星flux标记为 -1.0，排在输出列表最前面，质心精度优于1px。
...
|------|--------|--------|--------|------|------|
| Red帧(银心) | 4500×3600 | 163 | 13936 | 14099 | ~17s |
...
| 脚本 | 说明 |
|------|------|
| `python/test_detection.py` | 基础检测示例，生成红色十字标注图像 |
| `python/test_saturated.py` | 饱和星点检测示例，生成绿色十字+红色圆圈调试图像 |
